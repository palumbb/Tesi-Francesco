...
from model import evaluate, train_fednova


class FedNovaFlowerClient(fl.client.NumPyClient):

    def __init__(self, cid, trainloader, valloader, model_cfg, optimizer_cfg) -> None:
        super().__init__()
        self.cid = cid
    ...
    def fit(self, parameters, config):
        ...
        a_i, g_i = train_fednova(self.model, self.trainloader, optimizer, epochs, self.momentum, self.device)
        g_i_np = [param.cpu().numpy() for param in g_i]
        return g_i_np, len(self.trainloader), {"a_i": a_i}
